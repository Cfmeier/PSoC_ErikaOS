...
#include "`$INSTANCE_NAME`_eecfg.h"

#include "`$INSTANCE_NAME`_ee_rtdruid_versions.h"
...
/* ARM */
#ifdef __CORTEX_MX__
#include "`$INSTANCE_NAME`_cortex_mx_ee_internal.h"
#endif
...
#include "`$INSTANCE_NAME`_ee_oo_kernel.h"
#include "`$INSTANCE_NAME`_ee_as_kernel.h"
/* Moved inline interrupt services inclusion here, because they need to see TP
   declarations */
#include "`$INSTANCE_NAME`_ee_oo_inline.h"

/* XXX: IT CAN HAPPEN THAT SOME OSEK INLINES NEED SOME INTERNALS FROM AS LAYER */
#include "`$INSTANCE_NAME`_ee_as_internal.h"
#include "`$INSTANCE_NAME`_ee_oo_internal.h"
